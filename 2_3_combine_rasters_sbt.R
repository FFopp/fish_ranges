...
library(raster)
input_sdm <- '../fish_ranges_output_sdm_sbt'
...
for(i in method){
  ...
  for(spi in species_modelled){
    ...
    for(j in levels){
      output_dir <- file.path(paste0('../fish_ranges_output_sdm_sbt_', i), j)
...
for(i in method){
  for(j in levels){
    ...
    input_sdm <- file.path(paste0('../fish_ranges_output_sdm_sbt_', i), j)
    output_dir <- file.path('../fish_ranges_output_combined_sbt', i, j)
    if(!dir.exists(output_dir)){
      dir.create(output_dir, recursive = T)
    }
